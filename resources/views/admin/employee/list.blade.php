...
<script>
    ...
    var routes = {
        employees: {
            data: '{{ route_uri('employees.data') }}',
            create: '{{ route_uri('employees.create') }}',
            edit: '{{ route_uri('employees.edit') }}',
            delete: '{{ route_uri('employees.delete') }}',
        }
    };
    ...
    layui.config({
        ...
        table.on('toolbar(data-table)', function(obj){
            switch (obj.event) {
                case 'create': {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.employees.create), function(){
        ...
        table.on('tool(data-table)', function(obj){
            switch (obj.event) {
                case 'edit': {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.employees.edit, {id: obj.data.id}), function(){
                        table.reload('data-table');
                    });
                }break;
                case 'delete': {
                    layer.confirm('{{ trans('tips.table delete confirm') }}', function(index){
                        $.ajax({
                            type: 'DELETE',
                            url: route(routes.employees.delete),
                            data: {id: obj.data.id},
                            dataType: 'json',
                            async: false,
                            headers: {
