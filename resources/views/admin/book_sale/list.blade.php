    ...
    <div class="layui-row layui-col-space15">
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">书籍销售列表</div>
                <div class="layui-card-body">

                    <form class="" id="table-search-form" style="margin-bottom: 10px;">

                        <div class="layui-inline">
                            <input class="layui-input" name="name" autocomplete="off" placeholder="书籍销售名称">
                        </div>
                    ...
                    <script type="text/html" id="table-toolbar">
                        <div class="layui-btn-container">
                            <button class="layui-btn layui-btn-sm" lay-event="create">新增书籍销售</button>
...
<script>
    ...
    var routes = {
        book_sales: {
            data: '{{ route_uri('book_sales.data') }}',
            create: '{{ route_uri('book_sales.create') }}',
            edit: '{{ route_uri('book_sales.edit') }}',
            delete: '{{ route_uri('book_sales.delete') }}',
        }
    };
    ...
    layui.config({
        ...
        table.on('toolbar(data-table)', function(obj){
            switch (obj.event) {
                case 'create': {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.book_sales.create), function(){
        ...
        table.on('tool(data-table)', function(obj){
            switch (obj.event) {
                case 'edit': {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.book_sales.edit, {id: obj.data.id}), function(){
                        table.reload('data-table');
                    });
                }break;
                case 'delete': {
                    layer.confirm('{{ trans('tips.table delete confirm') }}', function(index){
                        $.ajax({
                            type: 'DELETE',
                            url: route(routes.book_sales.delete),
                            data: {id: obj.data.id},
                            dataType: 'json',
                            async: false,
                            headers: {
