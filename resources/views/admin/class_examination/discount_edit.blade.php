    ...
    <div class="layui-row layui-col-space15">
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">编辑新考试优惠</div>
                ...
                <form class="layui-form" action="">
                            ...
                            <div class="layui-input-block">
                                <select name="discount_type_id" id="discount-type-id">
                                    ...
                                    @foreach($types as $type)
                                        ...
                                        @else
                                            <option @if($discount->discount_type_id === $type->id) selected @endif value="{{ $type->id }}">{{ $type->name }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="layui-form-item">
                        <div class="layui-col-md6">
                            ...
                            <div class="layui-input-block">
                                <input type="text" class="layui-input" name="amount" value="{{ $discount->amount }}" id="amount" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="layui-form-item">
                        <div class="layui-col-md3">
                            ...
                            <div class="layui-input-block">
                                <input type="radio" @if($discount->status === STATUS_ON) checked @endif name="status" id="status" value="1" title="启用" />
                                <input type="radio" @if($discount->status === STATUS_OFF) checked @endif name="status" id="status" value="0" title="停用" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="layui-form-item">
                        ...
                        <div class="layui-input-block">
                            <textarea class="layui-textarea" name="remark" id="remark" placeholder="请输入考试优惠备注">{{ $discount->remark }}</textarea>
                        </div>
                    </div>
...
<script>
    ...
    var routes = {
        class_examination_discounts: {
            update: '{{ route_uri('class_examination_discounts.update') }}',
        }
    };
    ...
    layui.use(['form', 'laydate'], function(){
        ...
        form.on('submit(form-submit)', function(obj){
            $.ajax({
                type: 'PUT',
                url: route(routes.class_examination_discounts.update),
                data: obj.field,
                dataType: 'json',
                async: false,
