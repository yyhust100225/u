                ...
                <div class="layui-card-body">
                    ...
                    <form class="" id="table-search-form" style="margin-bottom: 10px;">
                        ...
                        <div class="layui-inline">
                            <input class="layui-input" name="name" autocomplete="off" placeholder="城市名称">
                        </div>
                    ...
                    <script type="text/html" id="table-toolbar">
                        <div class="layui-btn-container">
                            <button class="layui-btn layui-btn-sm" lay-event="create">新增城市</button>
...
<script>
    ...
    var routes = {
        cities: {
            data: '{{ route_uri('cities.data') }}',
            create: '{{ route_uri('cities.create') }}',
            edit: '{{ route_uri('cities.edit') }}',
            delete: '{{ route_uri('cities.delete') }}',
        }
    };
    ...
    layui.config({
        ...
        table.on('toolbar(data-table)', function(obj){
            switch (obj.event) {
                case 'create': {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.cities.create), function(){
        ...
        table.on('tool(data-table)', function(obj){
            switch (obj.event) {
                case 'edit': {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.cities.edit, {id: obj.data.id}), function(){
                        table.reload('data-table');
                    });
                }break;
                case 'delete': {
                    layer.confirm('{{ trans('tips.table delete confirm') }}', function(index){
                        $.ajax({
                            type: 'DELETE',
                            url: route(routes.cities.delete),
                            data: {id: obj.data.id},
                            dataType: 'json',
                            async: false,
                            headers: {
