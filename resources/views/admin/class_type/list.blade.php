    ...
    <div class="layui-row layui-col-space15">
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">班型列表</div>
                <div class="layui-card-body">
                    ...
                    <form class="" id="table-search-form" style="margin-bottom: 10px;">
                        ...
                        <div class="layui-inline">
                            <label>
                                <input class="layui-input" name="name" autocomplete="off" placeholder="班型标题">
                            </label>
                        </div>
                    ...
                    <script type="text/html" id="table-toolbar">
                        <div class="layui-btn-container">
                            <button class="layui-btn layui-btn-sm" lay-event="create">新增班型</button>
...
<script>
    ...
    let routes = {
        class_types: {
            data: '{{ route_uri('class_types.data') }}',
            create: '{{ route_uri('class_types.create') }}',
            edit: '{{ route_uri('class_types.edit') }}',
            delete: '{{ route_uri('class_types.delete') }}',
            discounts: "{{ route_uri('class_type_discounts.list') }}",
        }
    };
    ...
    layui.config({
        ...
        table.on('toolbar(data-table)', function(obj){
            switch (obj.event) {
                case 'create': {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.class_types.create));
                }break;
                case 'refresh': {
        ...
        table.on('tool(data-table)', function(obj){
            switch (obj.event) {
                case 'discounts' : {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.class_types.discounts, {id: obj.data.id}));
                }break;
                case 'edit': {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.class_types.edit, {id: obj.data.id}));
                }break;
                case 'delete': {
                    layer.confirm('{{ trans('tips.table delete confirm') }}', function(index){
                        $.ajax({
                            type: 'DELETE',
                            url: route(routes.class_types.delete),
                            data: {id: obj.data.id},
                            dataType: 'json',
                            async: false,
                            headers: {
