    ...
    <div class="layui-row layui-col-space15">
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">班型优惠列表</div>
                <div class="layui-card-body">
                    ...
                    <form class="" id="table-search-form" style="margin-bottom: 10px;">
                        ...
                        <div class="layui-inline">
                            <label>
                                <input class="layui-input" name="name" autocomplete="off" placeholder="优惠名称">
                            </label>
                        </div>
                    ...
                    <script type="text/html" id="table-toolbar">
                        <div class="layui-btn-container">
                            <button class="layui-btn layui-btn-sm" lay-event="create">新增班型优惠</button>
...
<script>
    ...
    let routes = {
        class_type_discounts: {
            data: '{{ route_uri('class_type_discounts.data') }}',
            create: '{{ route_uri('class_type_discounts.create') }}',
            edit: '{{ route_uri('class_type_discounts.edit') }}',
            delete: '{{ route_uri('class_type_discounts.delete') }}',
        }
    };
    ...
    layui.config({
        ...
        table.render({
            ...
            url: route(routes.class_type_discounts.data),
            where: {type_id: type_id},
            cols: [[
                // {field:'id', title: 'ID', width:'4%', sort: true, fixed: 'left'},
                {field:'discount_name', align:'center', title: '优惠名称'},
                {field:'effect_time', align:'center', title: '生效时间'},
                {field:'amount', align:'center', width:'8%', title: '优惠金额'},
        ...
        table.on('toolbar(data-table)', function(obj){
            switch (obj.event) {
                case 'create': {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.class_type_discounts.create, {type_id: type_id}));
                }break;
                case 'refresh': {
        ...
        table.on('tool(data-table)', function(obj){
            switch (obj.event) {
                case 'edit': {
                    makeLayerForm(layer, '{{ trans('tips.layer form title') }}', route(routes.class_type_discounts.edit, {id: obj.data.id}));
                }break;
                case 'delete': {
                    layer.confirm('{{ trans('tips.table delete confirm') }}', function(index){
                        $.ajax({
                            type: 'DELETE',
                            url: route(routes.class_type_discounts.delete),
                            data: {id: obj.data.id},
                            dataType: 'json',
                            async: false,
                            headers: {
